...
class RemoteNodeInternal {
    constructor(nodeType, val, cloneIndex) {
        if(typeof cloneIndex==='undefined'){
            index++
        }else{
            index = cloneIndex
        }
    ...
    cloneNode (deep){
        return new RemoteNodeInternal(this.nodeType, this.$value, this.$index)
    }
    toString() {
...
class RemoteElement extends RemoteNode {
    ...
    set innerHTML(val) {
        ...
        this.$innerHTML = val;
        //TODO: 修复无法获取textContent, 暂时先用innerHTML代替
        this.$textContent = val;
    }
...
let documentElement = new RemoteElement('html');

/** head Element */
class Head extends RemoteElement {
    constructor() {
        super('head');
        this.parentNode = documentElement;
        this.$index = Constants.HEAD;
    }
}

/** head Element */
/** body Element */
class Body extends RemoteElement {
    constructor() {
        super('body');
        this.parentNode = documentElement;
        this.$index = Constants.BODY;
...
function createContainer(name) {
    ...
    connectedElementsByIndex[Constants.CONTAINER] = this;
...
function dispatchEvent(target, host, event) {
    addToQueue(Commands.dispatchEvent, host, [target, event._type, event._eventInit, event._isCustom || false, event.newURL, event.oldURL]);
}
...
function setChannel(channel, timerFunction) {
    const initMsgPromise = new Promise(resolve => {
        initMsgPromiseResolver = resolve;
    });
...
updateBOM({
    url: self.rootPageURL || JSON.parse(atob(Util.parseQueryString(self.location.search.substr(1)).bom)).url
})
...
const window = {
    ...
    location: location,
    navigator: {
        userAgent: 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/54.0.2840.98 Safari/537.36 WebWorker'
    },
    history: new History(),
...
connectedElementsByIndex[Constants.HEAD] = head;
connectedElementsByIndex[Constants.BODY] = body;
