  ...
  constructor (channel, handler) {
    ...
    this.channel.addEventListener( 'message', function (ev) {
      ...
      if (message[Constants.REMOTE_DOM]) {
        ...
        //   requestAnimationFrame(()=>handler(message[Constants.REMOTE_DOM]))
      }else if(message[Constants.JSBRIDGE]){
        let jsbridgeAction = message[Constants.JSBRIDGE];
        self.jsbridge[jsbridgeAction[0]](jsbridgeAction[1])
      }
    });

  }

  postMessage (messages, regionName) {
    regionName = regionName || Constants.REMOTE_DOM;
    // console.log(messages, 'send messages')
    // let data = JSON.stringify({REMOTE_DOM: messages});

    //   let data = '{"REMOTE_DOM":[]}';
    this.channel.postMessage(JSON.stringify({[regionName]: messages}), self.pageId);//oc 必须序列化传值
      // console.log('SEND',messages)
    // this.channel.postMessage({REMOTE_DOM: messages});
  }
}
